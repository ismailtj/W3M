    ...
    def check_unique_formula(self):
        for line in self.line_ids:
            if line.is_matrix and len(line.formulas_ids) > 1:
                ...
                u"Une seule formule peut ête définie pour les tableaux (%s)!" % line.name)

    @api.multi
    def report_print(self):
        self.ensure_one()
        return self.env['ir.actions.report']._get_report_from_name('report.engin.xlsx').report_action(self)
...
class partner_xlsx(models.AbstractModel):
    _name = 'report.report.engin.xlsx'
