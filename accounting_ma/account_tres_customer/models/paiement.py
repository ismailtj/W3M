...
class PaiementEffetModelClient(models.Model):
    ...
    company_id = fields.Many2one('res.company', string=u'Société',
                                 default=lambda self: self.env['res.company']._company_default_get('paiement.effet.model.client'))
...
class PaiementChequeModelClient(models.Model):
    ...
    name = fields.Char(string='Nom', required=True)
    company_id = fields.Many2one('res.company', string=u'Société',
                                 default=lambda self: self.env['res.company']._company_default_get('paiement.cheque.model'))
    ...
    def action_post_entries(self):
        ...
        for effet in self:
            ...
            if effet.state == 'received':
                if effet.bordoreau_id:
                    date = effet.bordoreau_id.date
...
class PaiementChequeClient(models.Model):
    ...
    caisse_id = fields.Many2one('paiement.caisse', string=u'Caisse')
    paiement_record_id = fields.Many2one('paiement.record', string=u'Reçu de Paiement', ondelete='cascade')
    company_id = fields.Many2one('res.company', u'Société', required=True, default=lambda self: self.env['res.company']._company_default_get('paiement.cheque.client'))
    reco_amount = fields.Float(string=u'Montant lettré')
    analytic_account_id = fields.Many2one('account.analytic.account', string=u'Compte Analytique',
                                          states={'payed': [('readonly', True)]})
    rejete = fields.Boolean(string=u"Rejeté", copy=False, readonly=True, default=False)
    ...
    def action_post_entries(self):
        ...
        for cheque in self:
            ...
            if cheque.state == 'received':
                if cheque.bordoreau_id:
                    date = cheque.bordoreau_id.date
            ...
            if cheque.state == 'at_bank':
                ...
                deb_account = cheque.journal_id.default_credit_account_id.id
        ...
        for cheque in self:
            ...
            for move_obj in move_new_ids:
                for line in move_obj.line_ids:
                    credit = line.credit
                    debit = line.debit
                    line.write({'date': payed_date, 'credit': debit, 'debit': credit})
                move_obj.post()
    ...
    def action_to_represent(self):
        for cheque in self:
            caisse_centrale_id = self.env['paiement.caisse'].search([('caisse_centrale','=',True)],limit=1)
            default = {'cheque_origin_id':cheque.id,
...
class PaiementOvClient(models.Model):
    ...
    caisse_id = fields.Many2one('paiement.caisse', string=u'Caisse')
    paiement_record_id = fields.Many2one('paiement.record', string=u'Reçu de Paiement', ondelete='cascade')
    company_id = fields.Many2one('res.company', u'Société', required=True, default=lambda self: self.env['res.company']._company_default_get('paiement.ov.client'))
    analytic_account_id = fields.Many2one('account.analytic.account', string=u'Compte Analytique',
                                          states={'payed': [('readonly', True)]})
    origin = fields.Char('Origine')
...
class PaiementCashClient(models.Model):
    ...
    company_id = fields.Many2one('res.company', string=u'Société', required=True,
                                 states={'done':[('readonly', True)]}, default=lambda self: self.env['res.company']._company_default_get('paiement.cash.client'))
    ...
    def check_price_cb(self):
        ...
        if check_controle:
            if self.amount < float(min):
                error = u"Le montant du CB %s n'est pas accepeté car il est inférieur au minumum autorisé %s DH" % (self.amount,min)
