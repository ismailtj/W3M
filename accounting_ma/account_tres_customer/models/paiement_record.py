...
class TresFees(models.Model):
    ...
    def create_account_lines(self):
        ...
        for record in self:
            ...
            if record.bordoreau_id:
                journal_id = record.bordoreau_id.journal_id
                date = record.bordoreau_id.date
                name = record.bordoreau_id.name
                ref = record.bordoreau_id.name
                partner_id = False
...
class PaiementRecord(models.Model):
    ...
    @api.one
    @api.depends('paiement_lines.amount', 'invoice_ids.residual', 'invoice_ids.type')
    def _calc_amount(self):
        self.amount = sum(line.amount for line in self.paiement_lines)
        self.amount_invoices = sum(invoice.residual for invoice in self.invoice_ids if invoice.type == 'out_invoice')
        self.amount_avoirs = sum(avoir.residual for avoir in self.invoice_ids if avoir.type == 'out_refund')
        self.amount_diff = self.amount + self.amount_avoirs - self.amount_invoices
    ...
    invoice_ids = fields.Many2many('account.invoice', 'paiement_record_invoice_rel', 'paiement_record_id','invoice_id', u'Factures', states={'done':[('readonly', True)]})
        ...
        for rec in self:
            ...
            for invoice in rec.invoice_ids:
                if invoice.type == 'out_invoice':
                    ...
                    montant_invoice+= invoice.residual
