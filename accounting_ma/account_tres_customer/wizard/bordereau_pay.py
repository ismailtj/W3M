...
class ChequeClientBordoreauWizard(models.TransientModel):
    _name = "cheque.client.bordoreau.wizard"
    ...
    bor_id = fields.Many2one('bordoreau.pay','Bordereau', readonly=True)
    date = fields.Date('Date d\'encaissement', required=True, default=fields.Date.today())


class EffetClientBordoreauWizard(models.TransientModel):
    _name = "effet.client.bordoreau.wizard"
...
class BordereauPay(models.TransientModel):
    ...
    cheque_lines = fields.One2many('cheque.client.bordoreau.wizard', 'bor_id', 'Cheques')
    effet_lines = fields.One2many('effet.client.bordoreau.wizard', 'bor_id', 'Effets')
    ...
    def default_get(self, fields):
        ...
        bor = self.env['paiement.bordoreau'].browse(bor_id)
    ...
    def pay_action(self):
        for wizard in self:
            ...
            for cheque in wizard.cheque_lines:
                test = self.env['cheque.client.bordoreau.wizard'].browse(cheque.id)
