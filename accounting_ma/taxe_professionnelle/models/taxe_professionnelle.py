    ...
    def generate_data(self):
        ...
        for record in self:
            ...
            domain_terrains = [('date', '>=', self.fiscal_year_id.date_start),
                                ...
                                '|',('category_id.account_asset_id.code', 'like', '231%'),('category_id.account_asset_id.code', 'like', '232%'),
                                ('state', '=', 'open')]
            asset_terrains_ids = account_asset_obj.search(domain_terrains)
            for terrain in asset_terrains_ids:
                if terrain.category_id.account_asset_id.code.startswith('231') and not terrain.category_id.account_asset_id.code.startswith('2316'):
                    nature = 'terrains'
                elif terrain.category_id.account_asset_id.code.startswith('232') and not terrain.category_id.account_asset_id.code.startswith('2327'):
            ...
            domain_materiel = [('date', '>=', self.fiscal_year_id.date_start),
                                ('date', '<=', self.fiscal_year_id.date_end),
                                ('asset_succursale_id', '<=', self.asset_succursale_id.id),
                                '|',('category_id.account_asset_id.code', 'like', '233%'),
                                ('category_id.account_asset_id.code', 'like', '235%'),
                                ('state', '=', 'open')]
            asset_materiel_ids = account_asset_obj.search(domain_materiel)
            for materiel in asset_materiel_ids:
                if not materiel.category_id.account_asset_id.code.startswith('2351'):
                    materiel_vals = {
                            'name':materiel.name,
            ...
            for cession in asset_cessions_ids:
                if not cession.category_id.account_asset_id.code.startswith('234') and not cession.category_id.account_asset_id.code.startswith('2351'):
                    cession_vals = {
                            'name':cession.name,
