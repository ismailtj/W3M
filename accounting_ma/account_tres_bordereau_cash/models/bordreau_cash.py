...
class AccountMoveLine(models.Model):
    ...
    bordoreau_cash_id = fields.Many2one('paiement.bordoreau.cash', 'Bordoreau cash')


class PaiementCashClient(models.Model):
    _inherit = "paiement.cash.client"

    bordoreau_cash_id = fields.Many2one('paiement.bordoreau.cash', 'Bordoreau cash', copy=False)

    @api.multi
    def unlink(self):
        for rec in self:
            if rec.bordoreau_cash_id and rec.bordoreau_cash_id.state != 'draft':
...
class PaiementBordoreauCash(models.Model):
    _name = 'paiement.bordoreau.cash'
    _description = 'Bordoreau'
    ...
    cash_lines = fields.One2many('paiement.cash.client', 'bordoreau_cash_id', string=u'Espèces', copy=False)
    ...
    move_lines = fields.One2many('account.move.line', 'bordoreau_cash_id', 'Ecritures Comptables')
